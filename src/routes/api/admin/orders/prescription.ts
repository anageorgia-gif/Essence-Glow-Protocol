...
function missingColumnMessage(error: { message?: string; code?: string } | null) {
  const message = error?.message ?? "";
  if (
    error?.code === "42703" ||
    message.includes("prescription_pdf_path") ||
    message.includes("prescription_pdf_filename")
  ) {
        ...
        try {
          ...
          if (uploadError) {
            ...
            return jsonResponse(
              {
                error:
                  "Falha ao enviar o PDF para o Storage. Verifique o bucket 'protocols' e a chave service role.",
              },
              500,
            );
          }
            ...
            return jsonResponse(
              {
                error:
                  columnHint ??
                  "Falha ao salvar os metadados da prescrição no pedido.",
              },
              500,
            );
          }
          ...
          if (message.includes("SUPABASE_SERVICE_ROLE_KEY")) {
            return jsonResponse(
              {
                error:
                  "SUPABASE_SERVICE_ROLE_KEY não configurada no servidor. Conecte o Supabase no ambiente de deploy.",
              },
              500,
            );
          }
